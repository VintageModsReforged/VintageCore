...
public class ElectricHelper {
    ...
    public static ItemStack getCharged(Item item, int charge) {
        try {
            if (!Utils.instanceOf(item, "ic2.api.IElectricItem")) {
                throw new IllegalArgumentException(item + " must be an instanceof IElectricItem!");
            }
            ItemStack ret = new ItemStack(item);
            Class<?> electricClass = Class.forName("ic2.core.item.ElectricItem");
            Method chargeMethod = electricClass.getMethod(
                    "charge",
                    ItemStack.class, int.class, int.class, boolean.class, boolean.class
            );
            chargeMethod.invoke(null, ret, charge, Integer.MAX_VALUE, true, false);
            return ret;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
