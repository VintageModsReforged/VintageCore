...
public enum ChatFormatting implements StringRepresentable {
    ...
    private static final Map<String, ChatFormatting> FORMATTING_BY_NAME = new HashMap<String, ChatFormatting>();

    static {
        for (ChatFormatting value : values()) {
            FORMATTING_BY_NAME.put(cleanName(value.name), value);
        }
    }
    ...
    public static Collection<String> getNames(boolean hasColor, boolean hasFormat) {
        List<String> list = Lists.newArrayList();

        for(ChatFormatting chatformatting : values()) {
            if ((!chatformatting.isColor() || hasColor) && (!chatformatting.isFormat() || hasFormat)) {
